...
t_data = pd.read_csv("models/clean_data.csv")

# database configuration---------------------------------------
app.secret_key = "anuragkhilarescripts"
...
def content_based_recommendations(t_data, item_name, top_n=10):
    # Check if the item name exists in the training data
    if item_name not in t_data['Name'].values:
    ...
    tfidf_matrix_content = tfidf_vectorizer.fit_transform(t_data['Tags'])

    # Calculate cosine similarity between items based on descriptions
    cosine_similarities_content = cosine_similarity(tfidf_matrix_content, tfidf_matrix_content)

    # Find the index of the item
    item_index = t_data[t_data['Name'] == item_name].index[0]
    ...
    recommended_items_details = t_data.iloc[recommended_item_indices][['Name', 'ReviewCount', 'Brand', 'ImageURL', 'Rating']]
...
def recommendations():
    if request.method == 'POST':
        ...
        content_based_rec = content_based_recommendations(t_data, prod, top_n=nbr)
